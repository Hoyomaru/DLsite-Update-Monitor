...
> [!NOTE]
> この文書は、**v1.0.0正式公開前に使用していたv0.1.0の検証証跡**として保持しています。現在の正式公開Versionはv1.0.0です。以下の`.pext`名とSHA-256はv0.1.0専用であり、v1.0.0へ流用しません。
...
v1.0.0公開前には、`Validate-Build.ps1`、必要な実機Smoke Test、`Package-Release.ps1`、v1.0.0 `.pext`インストール確認を実行し、新しいSHA-256を生成してください。

安全性を優先し、DLsiteの`更新情報`と`ファイル容量`のみを監視します。比較基準は最後にユーザーが確認済みとしたリモートスナップショットです。

自動ダウンロード、自動パッチ適用、ローカル実行ファイルからのバージョン推定はv1.0.0でも対象外です。
